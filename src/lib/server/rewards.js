/**
 * Progression seals: badge milestones computed from a member's profile and
 * submissions, and the summaries shown on the rewards page and public profiles.
 *
...
export async function recomputeBadges(userId) {
	const [badges, earned, submissionRows, profile] = await Promise.all([
	...
	const metrics = sealMetrics(submissionRows, profile);
	const earnedIds = new Set(earned.map((b) => b.badge_id));
	const meets = (badge) =>
		BADGE_CRITERIA.includes(badge.criteria_type) &&
		metrics[badge.criteria_type] >= badge.criteria_value;
...
export async function getRewardsSummary(userId) {
	const [allBadges, earned, submissionRows, profile] = await Promise.all([
		listAllRows(TABLES.badges, [Query.equal('is_active', true)]),
		listAllRows(TABLES.userBadges, [Query.equal('user_id', userId)]),
		listSubmissionsByUser(userId),
		getProfileByUserId(userId)
	...
	const visibleBadges = allBadges
		.filter((b) => BADGE_CRITERIA.includes(b.criteria_type))
		.sort((a, b) => {
			const typeOrder =
				BADGE_CRITERIA.indexOf(a.criteria_type) - BADGE_CRITERIA.indexOf(b.criteria_type);
			return typeOrder || a.criteria_value - b.criteria_value;
		});
	const badges = visibleBadges.map((b) => {
		const earnedRow = earnedMap.get(b.$id);
		const current = metrics[b.criteria_type] ?? 0;
